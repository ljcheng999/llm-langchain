...
analysis_chain = RunnableParallel(
    sentiment=ChatPromptTemplate.from_template("What's the sentiment of: {text}")
    | model
    | parser,
    summary=ChatPromptTemplate.from_template("Summarize in 10 words: {text}")
    | model
    | parser,
    keywords=ChatPromptTemplate.from_template("Extract 3 keywords from: {text}")
    | model
    | parser,
)
