...
from mcp_state import AgentState
import os
...
def build_agent_graph(tools: List[BaseTool] = []):
    ...
    system_prompt = """
Your name is Misty and you are an expert data scientist. You help customers manage their data science projects by leveraging the tools available to you. Your goal is to collaborate with the customer in incrementally building their analysis or data modeling project. Version control is a critical aspect of this project, so you must use the git tools to manage the project's version history and maintain a clean, easy to understand commit history.
...
"""
    ...
    llm = ChatOpenAI(name="Misty", model="gpt-4.1-mini-2025-04-14")
    if tools:
    ...
    builder.add_node("Misty", assistant)
    builder.add_node(ToolNode(tools))

    builder.add_edge(START, "Misty")
    builder.add_conditional_edges(
        "Misty",
        tools_condition,
    )
    builder.add_edge("tools", "Misty")
