from tyler.graph import build_graph, AgentState
...
async def stream_graph_responses(
    input: dict[str, Any], graph: StateGraph, **kwargs
    ...
    async for message_chunk, metadata in graph.astream(
        input=input, stream_mode="messages", **kwargs
    ):
    ...
    try:
        ...
        while True:
            ...
            async for response in stream_graph_responses(
                graph_input, graph, config=config
            ):
            ...
            while thread_state.interrupts:
                ...
                for interrupt in thread_state.interrupts:
                    ...
                    interrupt_json_str = json.dumps(
                        interrupt.value, indent=2, ensure_ascii=False
                    )
            ...
            if user_input.lower() in ["exit", "quit"]:
                ...
                break
            graph_input = AgentState(
                messages=[HumanMessage(content=user_input)], yolo_mode=yolo_mode
            )
