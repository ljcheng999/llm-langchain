...
from tyler.mcps.config import mcp_config
from tyler.prompts import tyler_system_prompt
import json
...
async def build_graph():
    ...
    def assistant_node(state: AgentState) -> AgentState:
        response = llm.invoke(
            [SystemMessage(content=tyler_system_prompt)] + state.messages
        )
    ...
    def human_tool_review_node(
        state: AgentState,
    ) -> Command[Literal["assistant_node", "tools"]]:
    ...
    def assistant_router(state: AgentState) -> str:
        ...
        if not last_message.tool_calls:
            ...
            if not state.yolo_mode:
                if any(
                    tool_call["name"] in state.protected_tools
    ...
    builder.add_conditional_edges(
        "assistant_node", assistant_router, ["tools", "human_tool_review_node", END]
    )
