...
from mcp_servers.weather_server import mcp as weather
...
# 1: Initialize MultiServerMCPClient with both servers
# Add both calculator and weather server configurations
...
async def run_multi_server_agent():
    ...
    # 2: Get all tools from both servers
    # Use client.get_tools()
    tools = await client.get_tools()

    print(
        f"✅ Loaded {len(tools) if hasattr(tools, '__len__') else 'multiple'} tools from MCP servers"
    )

    # 3: Create react agent with all tools
    # Pass model and tools to create_agent
    ...
    calc_response = await agent.ainvoke({"messages": "What is 42 plus 58?"})
