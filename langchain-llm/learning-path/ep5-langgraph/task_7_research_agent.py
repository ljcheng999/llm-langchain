...
try:
    ...
    print(
        "Warning: DuckDuckGo search not available. Install with: pip install duckduckgo-search"
    )
...
# 2: Complete the calculator_tool
# Return result with "result" key
...
# 3: Complete the search_tool
# Use ddgs.text() to search
def search_tool(state: State):
    ...
    try:
        ...
        if "langgraph" in state["query"].lower():
            return {
                "result": "LangGraph is a framework for building stateful AI agents with graphs."
            }
        return {
            "result": f"Search unavailable, but I can tell you: {state['query']} is an interesting topic!"
        }
...
workflow.add_conditional_edges(
    "classify",
    router,
    {"calculator_tool": "calculator_tool", "search_tool": "search_tool"},
)
...
math_result = app.invoke(
    {"query": "What is 156 divided by 12?", "query_type": "", "result": ""}
)
...
search_result = app.invoke(
    {"query": "What is LangGraph used for?", "query_type": "", "result": ""}
)
