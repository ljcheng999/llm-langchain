...
def main():
    ...
    # 2: Build chain with list parser
    list_chain = list_prompt | llm | list_parser

    # Test the list chain
    if list_chain:
        result = list_chain.invoke({"technology": "cloud computing"})
    ...
    # 3: Build chain for JSON output
    json_chain = json_prompt | llm | json_parser

    # Test the JSON chain
    if json_chain:
        result = json_chain.invoke({"technology": "machine learning"})

        print(f"✅ Input: 'Analyze machine learning'")

        try:
            # 4: JsonOutputParser returns dict directly - no parsing needed!
            parsed = result
