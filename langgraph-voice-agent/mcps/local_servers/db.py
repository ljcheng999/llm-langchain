...
class DBExpense(Base):
    ...
    customer: Mapped["DBCustomer"] = relationship(
        back_populates="expenses", foreign_keys=[customer_id]
    )
...
# If the expense doesn't fit into any of the predefined categories, it should be classified as "other". This allows for flexibility in handling expenses that may not neatly fit into the specified categories, while still maintaining a structured approach to categorization.
...
async def update_expense(
    ...
    with SessionLocal() as session:
        expense = (
            session.query(DBExpense)
            .filter(DBExpense.id == id and DBExpense.customer_id == customer_id)
            .first()
        )
        if not expense:
            return "Expense not found"
...
async def list_expenses(customer_id: UUID) -> str:
    ...
    with SessionLocal() as session:
        ...
        expenses = [
            Expense.model_validate(expense.__dict__).model_dump_json(indent=2)
...
async def query_db(query: str, customer_id: UUID) -> str:
    ...
    with SessionLocal() as session:
        session.execute(
            text("SET app.customer_id = :customer_id"),
            {"customer_id": str(customer_id)},
        )

        result = session.execute(text(query), {"customer_id": customer_id})

    return pd.DataFrame(result.all(), columns=result.keys()).to_json(
